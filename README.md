...
| `series.json` | unique `slug`, `tvdbId`; unique `tmdbId` when set | `status`: `watchlist` (not started) \| `watching` (in progress) \| `up-to-date` \| `paused` (stopped mid-way) \| `completed` (unique regular `watchedEpisodes` cover `numberOfEpisodes`) \| `abandoned` (stopped mid-way for good). 100% coverage is always `completed`, including rows imported as abandoned. Incomplete `completed` demotes to paused. Rewatches do not add. `watchedEpisodes[].season` / `episode` integers ≥ 1. `startedAt` ≤ `finishedAt` when both set. Optional `reviewSlug` matches `src/content/reviews/series/{slug}.md`. |
